...
const ChairmanMessageSection = ({ filesList, contentData }) => {
  const image =
    getFileSettingsValue(
      filesList,
      contentData?.["about-us-section-4-image"] || ""
    )?.fileUrl || null;
  const heading = contentData?.["about-us-section-4-heading"] || "";
  const description = contentData?.["about-us-section-4-description"] || "";

  // Required data if any is not provided, return null
  if (!image && !heading && !description) return null;

  return (
    <div className="w-full px-[8px] md:px-[48px] flex flex-col lg:flex-row lg:justify-between lg:items-center gap-[24px] md:gap-[53px]">
      {image && (
